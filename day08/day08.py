...
class Instruction:
    def __init__(self, register_name: str, increase_factor: int, increase_value: int, condition_register: str, condition_lambda: Callable[[str], bool]):
    ...
    @classmethod
    def parse_condition(cls, condition_value: int, operator: str) -> Callable[[str], bool]:
        return {
    ...
    def parse(cls, line: str) -> object:
        ...
        return Instruction(register_name, increase_factor, increase_value, condition_register, condition_lambda)
...
def parse_instructions(lines: list[str]) -> list[Instruction]:
    return map(Instruction.parse, lines)
